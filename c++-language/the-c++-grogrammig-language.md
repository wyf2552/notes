...
# 构造函数
构造函数的定义
1. 在声明一个对象后，紧接着要给对象初给化
2. 对象初始化实质上就是对所有数据成员赋值
3. 如果对象中某个数据成员未赋值，则该数据成员的值不确定，那么该对象的值不完整
4. 构造函数（Constructor）用于创建一个对象，提供了初始化该对象的一种简便手段
5. 注意在前面所有类的定义中都有一个成员函数完成初始化数据成员，这个函数就可以当成构造函数
6. 构造函数的语法格式：
    ```c
    <类名〉（〈参数表〉）
    {
...
1. 在前面章节中的程序声明变量
    1. int a=0; #int a(0) ;
    2. struct Date today=12013, 3, 19｝；
    3. 或struct Date today （2013, 3, 19）；
    4. 以上为变量声明的同时赋初值，即都允许初始化
2. 对于天构造函数的类，育明变量（对象）不允许初始化口
    1. 例如 CLOCK alarm=｛10, 53, 11｝；//err
    2. CLOCK alarm （10, 53, 11）// err
3. 有构造函数就允许初始化
    1. 例如：Date birthday （1998, 12, 12）；// right
    2. Date birthday=11998, 12, 125 ; // err
