...
1. 一位全加器
全加器（FA）是最基本的加法单元，有加数Ai,加数Bi与低位传来的进位Ci-1共三个输⼊，有本位和Si及向⾼位的进位Ci共两个输出。

全加器的逻辑表达式如下:

和表达式：Si=Ai异或Bi异或Ci-1（当Ai,Bi,Ci-1中有奇数个1时，S i=1，否则Si=0）

进位表达式：C i=AiBi+（Ai异或Bi）Ci-1

2. 串行进位加法器
将n个全加器相连可得到n位加法⾏波进位，称为串⾏进位加法器，串行进位也称行波进位，每级进位直接依赖于前⼀级的进位，即进位信号是逐级形成的。在串⾏进位加法器中，低位运算产⽣进位所需的时间将影响⾼位运算的时间。因此，串⾏进位加法器的最长运算时间主要是由进位信号的传递时间决定的，位数越多，延迟时间就越长，所以加快进位产⽣和提⾼传递的速度是关键。

3. 并行进位加法器
并⾏进位（也称先⾏进位）加法器可以加快进位产⽣的速度，进⽽提升加法运算产⽣结果的速度。作⽤是“并⾏产⽣进位”，即n位进位信息⼏乎是同时产⽣的。因此，相⽐于串⾏进位加法器，并⾏进位加法器提升了加法运算产⽣结果的速度。

4. 带标志加法器
需要在n位加法器的基础上增加⼀些逻辑电路，使其不仅能计算和/差，还能⽣成相应的标志信息：分别是OF、CF、SF和ZF，每个标志信息占1位

溢出标志OF=Cn异或Cn-1，⽤于判断有符号数的加減运算是否溢出，OF=0表⽰未溢出，OF=1表⽰溢出。符号标志SF=Fn-1，⽤于表⽰有符号数的加减运算结果的正负性，SF=0表⽰结果为正，SF=1表⽰结果为负。零标志Z F=1当且仅当F=0，表⽰加减运算的结果是否为0,ZF=1表⽰结果为0，ZF=0表⽰结果⾮0。进位/借位标志C F=Cout异或Cin,⽤于判断⽆符数的加减运算是否发生溢出，CF=0表示未发生溢出,CF=1表示溢出

5. 算数逻辑单元ALU
ALU是⼀种功能较强的组合逻辑电路，它能进⾏多种算术运算和逻辑运算。加、减、乘、除运算最终都能归结为加法运算，因此A L U的核⼼是带标志加法器，同时也能执⾏“与”，“或”，“⾮”等逻辑运算
![](./image/2.2.1-1.png)

### 2.2.2 定点数的移位运算
1. 逻辑移位
逻辑移位的规则：左移时，⾼位移出，低位补0；右移时，低位移出，⾼位补0。对于⽆符号整数的逻辑左移，若⾼位的1移出，则发⽣溢出。

2. 算术移位
算术移位的规则：左移时，⾼位移出，低位补0，若移出的⾼位不同于移位后的符号位，即左移前后的符号位不同，则发⽣溢出：右移时，低位移出，⾼位补符号位，若低位的1移出，则影响精度
...
补码运算的特点如下。
    ...
    3）符号位与数值位⼀起参与运算，加、减运算结果的符号位也在运算中直接得出。

    4）最终运算结果的⾼位丢弃，保留n+1位，运算结果亦为补码。

2. 溢出判别方法
上溢：正+正=负，下溢：负+负=正

两个正数相加，⽽结果的符号位却为1（结果为负）；⼀个负数减去⼀个正数，结果的符号位却为0（结果为正）。

补码定点数加减运算溢出判断的⽅法有3种。
（1）采⽤⼀位符号位

减法运算在机器中是⽤加法器实现的，因此⽆论是加法还是减法，只要参加操作的两个数的符号相同，结果又与原操作数的符号不同，则表⽰结果溢出。设A的符号为A，B的符号为B，运算结果的符号为S，则溢出逻辑表达式为
![](./image/2.2.3-2.png)

若V=0，表⽰⽆溢出；若V=1，表⽰有溢出。

（2）采⽤双符号位

双符号位法也称模4补码。运算结果的两个符号位Ss1Ss2相同，表⽰未溢出；运算结果的两个符号位Ss1Ss2不同，表⽰溢出，此时最⾼位符号位代表真正的符号。

符号位S s 1 S s 2的各种情况如下：

    ①Ss1Ss2=0 0：表⽰结果为正数，⽆溢出。
    ②S s1Ss2=0 1：表⽰结果正溢出。
    ③S s 1 S s 2=1 0：表⽰结果负溢出。
    ④S s 1 S s 2=1 1：表⽰结果为负数，⽆溢出。

溢出逻辑判断表达式为V=S s 1异或Ss 2，若=0，表⽰⽆溢出；若V=1，表⽰有溢出。

（3）采⽤⼀位符号位根据数值位的进位情况判断溢出

若符号位（最⾼位）的进位Cn与最⾼数位（次⾼位〉的进位Cr-1，相同，说明⽆溢出，否则说明有溢出。溢出逻辑判断表达式为V=Cn异或Cn-1,若V=0，表示无溢出；V=1，表示有溢出明 ⽆ 溢 出 ， 否 则 说明有溢出。溢出逻辑判断表达式为V=Cn异或Cn-1,若V=0，表示无溢出；V=1，表示有溢出
